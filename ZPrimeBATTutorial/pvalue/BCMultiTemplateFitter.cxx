...
// ---------------------------------------------------------
int BCMultiTemplateFitter::SetTemplate(const char* channelname, const char* processname, TObjArray* obj, double efficiency)
{
	// get channel index
	int channelindex = GetChannelIndex(channelname);
	...
	TObjArray* myobj = (TObjArray*)obj->Clone();
        double upperParBound = myobj->GetEntries();

	// add parameter index to container
	fMassParIndexContainer.push_back(GetNParameters());

        AddParameter("Mass", 0.0, upperParBound);
  	SetPriorConstant("Mass");

        std::cout<<"Add Mass Parameter with "<<upperParBound<<" reweight templates"<<std::endl;
	TH1D hist = (*(TH1D*)myobj);
	...
	// set histogram
	bctemplate->SetTemplateHistograms(obj);
...
double BCMultiTemplateFitter::Probability(int channelindex, int processindex, int binindex, std::vector<double>& parameters)
{
	...
	double probability=hist->GetBinContent(binindex);
  	
	//std::cout<<probability <<std::endl;
	//Do linear interpolation between neighbouring template histogram
	//if (processindex == 2)
	//{
	//   int parindex = fMassParIndexContainer[0]; // assume there is only 1 'mass' parameter 
	//   double shift = double(parameters.at(parindex) - int(parameters.at(parindex)));//4
	//   double slope = fChannelContainer[channelindex]->GetTemplate(processindex)->GetNextHistogram()->GetBinContent(binindex) - probability;
	//   probability = shift * slope + probability;
	//   //std::cout<<"Shift "<<shift<<" slope "<<slope<<std::endl;	      
	//}	
...
double BCMultiTemplateFitter::LogLikelihood(std::vector<double> parameters)
{
	...
	for (int ichannel = 0; ichannel < fNChannels; ++ichannel) {
		...
		// fix the reweight histo in the multi dim TObjArrays
	        FixReweightHistogram(ichannel, parameters);
...
// ---------------------------------------------------------
int BCMultiTemplateFitter::FixReweightHistogram(int ichannel, std::vector<double>& parameters)
{

	int parindex = fMassParIndexContainer[0]; // assume there is only 1 'mass' parameter 
	//std::cout<<"Mass parameter "<<parindex<<std::endl;
	int template_index = int(parameters.at(parindex));
	//std::cout<<"current best template index "<<template_index<<std::endl;
	int processindex = 2;
	//Set histogram objects 
	//std::cout<<fChannelContainer[ichannel]->GetTemplate(processindex)->GetProcessName()<<std::endl;
	TObjArray* obj = fChannelContainer[ichannel]->GetTemplate(processindex)->GetTemplateHistograms();
	//std::cout<<"1"<<std::endl;
	TH1D* hist = ((TH1D*)(TObjArray*)obj->At(template_index));
	TH1D* hist_next;
	if (template_index >= obj->GetEntries() - 1)
	  hist_next = ((TH1D*)(TObjArray*)obj->At(template_index));
	else
	  hist_next = ((TH1D*)(TObjArray*)obj->At(template_index+1));
	
	hist->Scale(1.0 / hist->Integral()); 
	hist_next->Scale(1.0 / hist_next->Integral()); 

	//std::cout<<"2"<<std::endl;
	fChannelContainer[ichannel]->GetTemplate(processindex)->SetHistogram(hist);
	fChannelContainer[ichannel]->GetTemplate(processindex)->SetNextHistogram(hist_next);
	//TObjArray* obj = fChannelContainer[channelindex]->GetTemplate(processindex)->GetHistogram();
	//fTemplateReweightHistogramContainerUp[index][j] = (TH1D*)obj_up[j];
	//fTemplateReweightHistogramContainerDn[index][j] = (TH1D*)obj_dn[j];
	//std::cout<<"2"<<std::endl;
