...
class BCMTFAnalysisFacility
{
	...
	public:
		...
		int PerformCalibrationAnalysis(const char* dirname, std::vector<double> default_parameters, int index, std::vector<double> parametervalues, int nensembles = 1000, double maxsig = 1.);
		...
		TTree* PerformEnsembleTest(std::vector<double> parameters, int nensembles, double maxsig); 

		// perform ensemble test based on varying sets of parameters
		TTree* PerformEnsembleTest(TTree* tree, int nensembles, double maxsig); 
