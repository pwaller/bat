...
// ---------------------------------------------------------
BCMTFAnalysisFacility::BCMTFAnalysisFacility(BCMultiTemplateFitter* mtf) : fRandom(new TRandom3(0))
																																				 , fFlagMCMC(false)
{
	fMTF = mtf;
};
...
std::vector<TH1D> BCMTFAnalysisFacility::BuildEnsemble(std::vector<double> parameters)
{
	...
	for (int ichannel = 0; ichannel < fMTF->GetNChannels(); ++ichannel) {
		...
		// get channel
		BCChannel* channel = fMTF->GetChannel(ichannel);
...
// ---------------------------------------------------------
TTree* BCMTFAnalysisFacility::PerformEnsembleTest(std::vector<double> parameters, int nensembles, double maxsig)
{
	// create new tree
	TTree* tree = 0;
	...
	return PerformEnsembleTest(tree, nensembles, maxsig); 
}

// ---------------------------------------------------------
TTree* BCMTFAnalysisFacility::PerformEnsembleTest(TTree* tree, int nensembles, double maxsig)
{
	...
	for (int i = 0; i < nparameters; ++i) {
		...
		tree_out->Branch("Lb",  &fLb,  "Lb/D");
		tree_out->Branch("Lsb",  &fLsb,  "Lsb/D");
		if (fFlagMCMC) {
			...
			tree_out->Branch(Form("5quantile_marginalized_%i", i), &out_5quantile_marginalized[i], Form("marginalized 5%% quantile of par. %i/D", i));
			tree_out->Branch(Form("10quantile_marginalized_%i", i), &out_10quantile_marginalized[i], Form("marginalized 10%% quantile of par. %i/D", i));
			tree_out->Branch(Form("16quantile_marginalized_%i", i), &out_16quantile_marginalized[i], Form("marginalized 16%% quantile of par. %i/D", i));
			tree_out->Branch(Form("84quantile_marginalized_%i", i), &out_84quantile_marginalized[i], Form("marginalized 84%% quantile of par. %i/D", i));
			tree_out->Branch(Form("90quantile_marginalized_%i", i), &out_90quantile_marginalized[i], Form("marginalized 90%% quantile of par. %i/D", i));
			tree_out->Branch(Form("95quantile_marginalized_%i", i), &out_95quantile_marginalized[i], Form("marginalized 95%% quantile of par. %i/D", i));
	...
	for (int iensemble = 0; iensemble < nensembles; ++iensemble) {
		...
		if (fFlagMCMC) {
			...
			fMTF->MCMCResetResults();
	   	        ...
	   	        //B hypothesis
	   	        fMTF->SetParameterRange(2,0.0,0.0);
	   	        // run MCMC again assuming no signal
	   	        fMTF->MarginalizeAll();     
	   	        fMTF->FindMode(fMTF->GetBestFitParameters());
	   	        std::vector<double> null_parameters; 
	   	        null_parameters = fMTF->GetBestFitParameters();
		        fLb = exp(fMTF->LogLikelihood(null_parameters));
	   	        
			//allow signal
			fMTF->SetParameterRange(2,0.0,maxsig);
			...
			//S+B hypothesis
			std::vector<double> test_parameters = fMTF->GetBestFitParameters();
		        fLsb = exp(fMTF->LogLikelihood(test_parameters));
	                
			std::cout<<"test hyp (LogLike) "<<fMTF->LogLikelihood(test_parameters) << " null (LogLike) "<<fMTF->LogLikelihood(null_parameters)<<std::endl;
			std::cout<<"test hyp "<<fLsb << " null "<<fLb<<std::endl;
                        std::cout<<"TS "<<-2*log(fLsb/fLb)<<std::endl;
		}
		else {
			// find mode
			fMTF->FindMode();
...
int BCMTFAnalysisFacility::PerformSingleChannelAnalyses(const char* dirname, const char* options)
{
	...
	std::vector<BCComparisonTool*> ctc; 
	std::vector<BCComparisonTool*> ctc_mcmc; 
	...
	for (int i = 0; i < nparameters; ++ i) {
		...
		BCComparisonTool* ct = new BCComparisonTool(fMTF->GetParameter(i)->GetName().c_str());
		BCComparisonTool* ct_mcmc = new BCComparisonTool(fMTF->GetParameter(i)->GetName().c_str());
	...
	for (int isystematic = 0; isystematic < nsystematics; ++isystematic) {
		// get systematic
		BCSystematic* systematic = fMTF->GetSystematic(isystematic); 
	...
	for (int i = 0; i < nparameters; ++ i) {
		// get comparison tool
		BCComparisonTool* ct = ctc.at(i);
		BCComparisonTool* ct_mcmc = ctc_mcmc.at(i);
	...
	if (flag_mcmc) {
		...
		// draw first one
		BCComparisonTool* ct_mcmc =  ctc_mcmc.at(0);
	...
	// ---- free memory ---- //
	for (int i = 0; i < nparameters; ++i) {
		BCComparisonTool* ct = ctc[i]; 
		BCComparisonTool* ct_mcmc = ctc_mcmc[i]; 
		delete ct;
		delete ct_mcmc;
	}
	ctc.clear();
...
// ---------------------------------------------------------
int BCMTFAnalysisFacility::PerformCalibrationAnalysis(const char* dirname, std::vector<double> default_parameters, int index, std::vector<double> parametervalues, int nensembles, double maxsig)
{
	...
	for (int ivalue = 0; ivalue < nvalues; ++ivalue) {
		...
		// create ensemble
		TTree* tree = PerformEnsembleTest(parameters, nensembles, maxsig); 
