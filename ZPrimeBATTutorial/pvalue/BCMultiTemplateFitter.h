...
class BCMultiTemplateFitter : public BCModel
{
	...
	public:
		...
		int SetTemplate(const char* channelname, const char* processname, TObjArray* obj, double efficiency = 1.);
		...
		int FixReweightHistogram(int ichannel, std::vector<double>& parameters);
		
		TObjArray* GetNestedObject(TObjArray* obj, int slot);

		int GetObjectNpar(TObjArray* obj);
 ...
 private:
		...
		// a container of parameter indeces for the mass scale
		std::vector<int> fMassParIndexContainer; 
